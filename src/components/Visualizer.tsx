...
const Visualizer: React.FC<VisualizerProps> = ({ agents, signals, width, height }) => {
  ...
  useEffect(() => {
    ...
    const link = g.append('g')
      ...
      .attr('stroke-opacity', (d: any) => {
        ...
        return d.isUrgent ? 0.8 : 0.4;
      })
      .attr('stroke-width', (d: any) => {
        const baseWidth = Math.sqrt(d.payload) * (d.isUrgent ? 3 : 2);
    ...
    node.append('circle')
      ...
      .attr('stroke', (d: any) => tracedAgentIds.has(d.id) ? '#fbbf24' : '#ecfdf5')
      .attr('stroke-width', (d: any) => tracedAgentIds.has(d.id) ? 4 : 2)
  ...
  }, [agents, signals, width, height, selectedSignalId, tracedSignalIds, tracedAgentIds]);

  return (
    <div className="relative w-full h-full bg-emerald-950 rounded-2xl overflow-hidden border border-emerald-800 shadow-2xl" onClick={() => setSelectedSignalId(null)}>
      {agents.length === 0 ? (
      ...
      <div className="absolute bottom-4 right-4 text-[10px] font-mono text-emerald-600 pointer-events-none">
        SCROLL TO ZOOM • DRAG TO PAN • CLICK SIGNAL TO TRACE
      </div>
